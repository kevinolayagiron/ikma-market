...
async function saveProduct(e){
 e.preventDefault(); formStatus.textContent="Guardando...";
 const payload={title:$("title").value.trim(),description:$("description").value.trim(),price:$("price").value.trim(),category:$("category").value.trim(),image_url:$("image_url").value.trim(),link:$("link").value.trim(),badge:$("badge").value.trim(),active:$("active").value==="true"};
 let result;
...
async function loadProducts(){
 ...
 list.innerHTML=data.map(p=>`<div class="product-row"><div class="row-info"><img class="thumb" src="${escapeAttr(p.image_url||"")}" onerror="this.style.display='none'"><div><strong>${escapeHtml(p.title)}</strong><div class="muted">${escapeHtml(p.category||"Sin categoría")} · ${escapeHtml(p.price||"Sin precio")} · ${p.active?"Visible":"Oculto"}</div></div></div><div class="actions"><button class="btn2 secondary" onclick="editProduct('${p.id}')">Editar</button><button class="btn2 danger" onclick="deleteProduct('${p.id}')">Eliminar</button></div></div>`).join("");
 window._products=data;
}
window.editProduct=id=>{
 const p=(window._products||[]).find(x=>x.id===id);if(!p)return;
 editingId=id;$("formTitle").textContent="Editar producto";$("cancelBtn").hidden=false;
 ["title","description","price","category","image_url","link","badge"].forEach(k=>$(k).value=p[k]||"");
 $("active").value=String(p.active);window.scrollTo({top:0,behavior:"smooth"});
};
...
function resetForm(){editingId=null;$("productForm").reset();$("active").value="true";$("formTitle").textContent="Añadir producto";$("cancelBtn").hidden=true}
